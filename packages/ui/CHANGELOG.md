...
- 8ce26f1: `SidebarSwitcher` takes the avatar's identity as `avatarId`, so the button keeps its own `id`.
...
- a83e666: `ClientModule` can declare `overlays`: components the shell mounts once per workspace and keeps.
...
- Updated dependencies [a83e666]
  - @kernhq/kernel@0.10.2
...
- e4be69b: Diagram and embed nodes, and a Mermaid renderer that runs on the server.
...
- 111bd4c: The framework's own words are seeded, not registered by an import side effect.
  ...
  Rollup duplicated the _data_ into six client chunks and kept the loop in one — which tracker's
...
- c3f94be: A page's reading column is centred, and a document body draws no focus ring.

  `maxWidth` on `Page` describes a _measure_, not a left margin — but `.inner` had no `margin-inline`,
...
- c733ee0: `{n}` and `{count}` are the same placeholder, because `count` and `n` are the same argument.
...
- 6ec42f6: `--kern-ink-250` clears 4.5:1 in both palettes, against the surface it actually sits on.
...
- 753eb5b: `setRouteParams` — so `navigation.params` carries the parameters a module's own route matched.
  ...
  props. Anything _outside_ the route component therefore read `undefined`.
