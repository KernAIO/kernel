 ...
 * An optional `topBar` (the window tab strip, DESIGN.md §2.0) spans every column above them.
 * ≤1024px: sidebar becomes a drawer (bind:sidebarOpen). ≤768px: rail and top bar hidden, bottom tab bar shown.
...
<div class={cn('kshell', topBar && 'has-top', hideSidebar && 'no-sidebar', sidebarOpen && 'drawer-open', className)}>
  {#if topBar}<div class="kshell-top">{@render topBar()}</div>{/if}
...
<style>
  ...
  .kshell.has-top { --kshell-top-h: var(--kern-tabbar-h); grid-template-rows: var(--kshell-top-h) minmax(0, 1fr); }
  .kshell-top { grid-column: 1 / -1; min-width: 0; }
  ...
  @media (max-width: 768px) {
    ...
    .kshell, .kshell.has-top { --kshell-top-h: 0px; grid-template-columns: minmax(0, 1fr); grid-template-rows: minmax(0, 1fr) auto; }
    .kshell-top { display: none; }
