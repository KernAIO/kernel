...
export const MailProviderConfig = z.discriminatedUnion('provider', [
  ...
  z.object({ provider: z.literal('smtp'), host: z.string(), port: z.number().int(), secure: z.boolean().default(true), user: z.string().optional(), pass: z.string().optional(), from: z.string() }),
  z.object({ provider: z.literal('mailgun'), apiKey: z.string(), domain: z.string(), region: z.enum(['us', 'eu']).default('us'), from: z.string() }),
  z.object({ provider: z.literal('ses'), accessKeyId: z.string(), secretAccessKey: z.string(), region: z.string(), from: z.string() }),
