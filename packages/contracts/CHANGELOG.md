...
- 5195ac9: Let a search document declare the permission its hit must clear.
...
subjects)` — so a **deny** binding is invisible to it and workspace search fails open. Measured
  ...
  No module can repair this from its own side: there is no subject string that _subtracts_, and
  ...
  and neither replaces the other: only `acl` knows a _readership_ (a narrow-scope `can()` with no
  binding falls through to the caller's workspace set and answers true), and only `authz` knows an
  administrator's _decision_.
