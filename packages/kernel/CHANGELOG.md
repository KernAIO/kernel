...
- 7538cca: Apply the guest deny-floor to a cached permission set, not only to a computed one.
...
- 47ed0b2: Add a demo-content extension point, so a new workspace can be created already full.
...
- Updated dependencies [47ed0b2]
  - @kernhq/contracts@0.8.1

## 0.10.3

### Patch Changes

- 12a4a69: `rlsPolicySql` now emits `drop policy if exists` before `create policy`, so a module migration
...
- 0541769: Treat a blank environment variable as unset, for every key `KernelEnv` declares. A compose file
...
- 49e4ae4: Read `S3_FORCE_PATH_STYLE` as a word rather than as truthiness. `z.coerce.boolean()` is
