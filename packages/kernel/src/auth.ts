...
export function createAuthVerifier(opts: { coreUrl: string; kernSecret: string; issuer?: string }): AuthVerifier {
  const jwks = createRemoteJWKSet(new URL('/api/auth/jwks', opts.coreUrl))
  const hs = createSecretKey(Buffer.from(opts.kernSecret))
  return {
    async verify(token) {
      try { const { payload } = await jwtVerify(token, jwks, { issuer: opts.issuer }); return payload } catch { return null }
    },
    async signService(service, ttlSec = 300) {
      return new SignJWT({ svc: service }).setProtectedHeader({ alg: 'HS256' }).setIssuedAt().setIssuer('kern-internal').setExpirationTime(`${ttlSec}s`).sign(hs)
    },
    async verifyService(token) {
      try { const { payload } = await jwtVerify(token, hs, { issuer: 'kern-internal' }); return (payload as any).svc ?? null } catch { return null }
    },
  }
}

export const systemPrincipal = (service: string): Principal => ({ ...ANONYMOUS, kind: 'service', service, instanceAdmin: true, name: `service:${service}` })
export function principalFromClaims(p: JWTPayload & Record<string, unknown>, memberships: MembershipSummary[] = []): Principal {
  return {
