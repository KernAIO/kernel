...
import { KernError, httpStatusFor } from './errors.js'
...
export const workspaceScoped = (moduleId?: string) =>
  o.middleware(async ({ context, next }, input: { workspaceId: string }) => {
    ...
    if (!principal.instanceAdmin && principal.kind !== 'service') kernel.authz.requireMember(principal, input.workspaceId)
    if (moduleId && !(await kernel.isModuleEnabled(input.workspaceId, moduleId))) throw new ORPCError('MODULE_DISABLED', { data: { module: moduleId } })
...
export const requires = (permission: string) =>
  o.middleware(async ({ context, next }, input: { workspaceId: string }) => {
    await context.kernel.authz.require(context.principal, permission, { kind: 'workspace', id: input.workspaceId, workspaceId: input.workspaceId })
    return next()
  })

export function kernErrorToORPC(err: unknown): unknown {
  if (err instanceof KernError) return new ORPCError(err.code, { message: err.message, data: err.details, status: httpStatusFor(err.code), cause: err })
  return err
}
...
export async function createHttpServer(opts: HttpOptions): Promise<FastifyInstance> {
  const { kernel } = opts
  const app = Fastify({ logger: false, trustProxy: true, bodyLimit: 25 * 1024 * 1024, genReqId: () => crypto.randomUUID() })
  await app.register(helmet, { contentSecurityPolicy: false, crossOriginResourcePolicy: { policy: 'cross-origin' } })
  await app.register(cors, { origin: opts.corsOrigins, credentials: true, methods: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'] })
  await app.register(rateLimit, { max: 600, timeWindow: '1 minute', allowList: (req) => req.headers['x-kern-service'] !== undefined })

  app.addHook('onRequest', async (req, reply) => { reply.header('x-request-id', req.id) })
  app.addHook('onResponse', async (req, reply) => {
    if (req.url.startsWith('/api/health')) return
    kernel.log.info({ method: req.method, url: req.url, status: reply.statusCode, ms: Math.round(reply.elapsedTime), reqId: req.id }, 'req')
  })
  app.setErrorHandler((err: unknown, req, reply) => {
    ...
    reply.status(status).send({ code: e?.code ?? (status === 429 ? 'RATE_LIMITED' : 'INTERNAL'), message: status >= 500 ? "Internal error" : error.message, details: e?.details })
  })

  app.get('/api/health', async () => ({ ok: true, service: kernel.service, version: kernel.version, modules: kernel.registry.ids() }))
  app.get('/api/ready', async (_req, reply) => {
    try { await kernel.database.pool.query('select 1'); return { ok: true } } catch { return reply.status(503).send({ ok: false }) }
  })
  ...
  for (const mod of kernel.registry.all()) {
    ...
    const rpc = new RPCHandler(router, { interceptors: [onError((e) => { if (e instanceof KernError) throw kernErrorToORPC(e) })] })
    const rest = new OpenAPIHandler(router, { interceptors: [onError((e) => { if (e instanceof KernError) throw kernErrorToORPC(e) })] })
    const spec = await generator.generate(router, {
      info: { title: `${opts.openapi?.title ?? 'Kern'} – ${mod.definition.name}`, version: mod.definition.version, description: mod.definition.description },
      servers: [{ url: prefix }],
      security: [{ bearerAuth: [] }],
    ...
    app.all(`${prefix}/*`, async (req, reply) => {
      ...
      const context: RequestContext = { kernel, principal, requestId: req.id, ip: req.ip, headers: req.headers as any }
      const isRpc = req.url.startsWith(`${prefix}/rpc`)
      const handler = isRpc ? rpc : rest
      const { matched } = await handler.handle(req.raw, reply.raw, { prefix: (isRpc ? `${prefix}/rpc` : prefix) as `/${string}`, context })
