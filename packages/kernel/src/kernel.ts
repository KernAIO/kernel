...
import { type Database, createDatabase } from './db.js'
import { type EventBus, type PublishOptions } from './events/bus.js'
import { NatsEventBus, createEventBus } from './events/nats.js'
import { type Jobs, createJobs } from './jobs.js'
import { type Logger, createLogger } from './logger.js'
import type { ServerModule } from './module.js'
import { toManifest } from './module.js'
import { type Realtime, createRealtime } from './realtime.js'
...
export async function createKernel(opts: KernelOptions): Promise<Kernel> {
  ...
  const redis = env.VALKEY_URL ? new Redis(env.VALKEY_URL, { lazyConnect: false, maxRetriesPerRequest: 2 }) : null
  ...
  const storage = createStorage({ endpoint: env.S3_ENDPOINT, publicEndpoint: env.S3_PUBLIC_ENDPOINT, region: env.S3_REGION, bucket: env.S3_BUCKET, accessKey: env.S3_ACCESS_KEY, secretKey: env.S3_SECRET_KEY, forcePathStyle: env.S3_FORCE_PATH_STYLE })
  ...
  const cache: AuthzCache | undefined = redis ? {
    ...
    del: async (prefix) => { const keys = await redis.keys(`${prefix}*`); if (keys.length) await redis.del(...keys) },
  } : undefined

  let kernel!: Kernel
  const jobs = await createJobs({ url: env.DATABASE_URL, log, kernel: () => kernel })
  const authzStoreFallback: AuthzStore = {
    customRolePermissions: (workspaceId, userId) => broker.call('core.authz.customRolePermissions', { workspaceId, userId }, system),
    bindings: (workspaceId, userId, groupIds, role) => broker.call('core.authz.bindings', { workspaceId, userId, groupIds, role }, system),
  }
  const authz = new Authz(null as unknown as AuthzStore, cache)

  kernel = {
    service: opts.service, version: opts.version ?? '0.0.0', role, env, log, registry, database, events, broker,
    authz, jobs, settings, secrets, storage, realtime, auth, redis, nats, system,
    ...
    async start() {
      ...
      for (const mod of registry.all()) {
        ...
        if (mod.procedures) broker.register(mod.definition.id, Object.fromEntries(Object.entries(mod.procedures).map(([k, p]) => [k, { input: p.input, output: p.output, handler: (input: unknown, ctx: { principal: Principal }) => p.handler(input, { kernel, principal: ctx.principal }) }])))
        if (mod.jobs?.length) jobs.register(mod.definition.id, mod.jobs)
        for (const [pattern, handler] of Object.entries(mod.subscriptions ?? {})) {
          await events.subscribe(pattern, (e: EventEnvelope) => handler(e, kernel), { durable: `${opts.service}-${mod.definition.id}-${pattern}` })
        }
      }
      // drop cached permissions on changes
      await events.subscribe('core.permissions.changed', async (e) => { const p = e.payload as { workspaceId: string; userIds: string[] | null }; if (p.userIds) for (const u of p.userIds) await authz.invalidate(p.workspaceId, u); else await authz.invalidate(p.workspaceId) })
      await events.subscribe('core.module.*', async (e) => settings.invalidate((e.payload as any).workspaceId))
    ...
    async stop() {
      ...
      await jobs.stop(); broker.close(); await events.close(); redis?.disconnect(); await database.close()
    },
  }
  return kernel
}
