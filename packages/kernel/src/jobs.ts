import { PgBoss, type Job, type SendOptions, type ScheduleOptions } from "pg-boss"
...
export async function createJobs(opts: { url: string; log: Logger; kernel: () => unknown }): Promise<Jobs> {
  ...
  return {
    boss,
    async send(name, data, o) { await boss.createQueue(name).catch(() => {}); return boss.send(name, data as object, o) },
    async schedule(name, cron, data = {}, o) { await boss.createQueue(name).catch(() => {}); await boss.schedule(name, cron, data, o) },
    register(module, defs) { for (const def of defs) registered.push({ name: `${module}.${def.name}`, def }) },
    async startWorkers() {
      for (const { name, def } of registered) {
        await boss.createQueue(name, { retryLimit: def.options?.retryLimit ?? 3, retryDelay: def.options?.retryDelay ?? 10, retryBackoff: def.options?.retryBackoff ?? true, expireInSeconds: def.options?.expireInSeconds ?? 900 }).catch(() => {})
        await boss.work(name, { batchSize: 1, pollingIntervalSeconds: 1 }, async ([job]: Job<unknown>[]) => {
          ...
          await def.handler(input, { kernel: opts.kernel() as any, id: job.id, attempt: (job as any).retryCount ?? 0 })
        })
        if (def.cron) await boss.schedule(name, def.cron, {}, { tz: 'UTC' })
        opts.log.info({ job: name, cron: def.cron }, 'job worker started')
      }
    },
    async stop() { await boss.stop({ graceful: true, timeout: 10_000 }) },
  }
}
