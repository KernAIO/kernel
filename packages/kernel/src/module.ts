...
import type { EventDef, ModuleManifest, PermissionDef, Principal } from "@kernalo/contracts"
...
export type EventHandler<P = unknown> = (event: import('@kernalo/contracts').EventEnvelope<P>, kernel: Kernel) => Promise<void> | void

export interface JobDef<TInput = unknown> {
  name: string
  schema?: z.ZodType<TInput>
  handler: (input: TInput, ctx: { kernel: Kernel; id: string; attempt: number }) => Promise<void>
  options?: { retryLimit?: number; retryDelay?: number; retryBackoff?: boolean; expireInSeconds?: number; singletonKey?: string; teamSize?: number }
  /** cron expression to run on a schedule (input = {}) */
  cron?: string
}

export interface AutomationTriggerDef { key: string; label: string; event: string; schema?: z.ZodTypeAny }
export interface AutomationConditionDef { key: string; label: string; schema: z.ZodTypeAny; evaluate: (input: unknown, ctx: { event: unknown; kernel: Kernel; workspaceId: string }) => Promise<boolean> | boolean }
export interface AutomationActionDef { key: string; label: string; schema: z.ZodTypeAny; run: (input: unknown, ctx: { event: unknown; kernel: Kernel; workspaceId: string; actor: Principal }) => Promise<unknown> }
...
export interface ObjectResolver {
  type: string
  resolve: (workspaceId: string, ids: string[], principal: Principal, kernel: Kernel) => Promise<Array<{ id: string; title: string; url: string; icon?: string | null; subtitle?: string | null } | null>>
}
...
export interface ServerModule<TSettings extends z.ZodTypeAny = z.ZodTypeAny> {
  ...
  jobs?: JobDef<any>[]
  automations?: { triggers?: AutomationTriggerDef[]; conditions?: AutomationConditionDef[]; actions?: AutomationActionDef[] }
  search?: SearchIndexer[]
  resolvers?: ObjectResolver[]
  /** procedures other modules/services may call via kernel.call('<module>.<name>', input) */
  procedures?: Record<string, { input?: z.ZodTypeAny; output?: z.ZodTypeAny; handler: (input: any, ctx: { kernel: Kernel; principal: Principal }) => Promise<any> }>
  /** lifecycle */
...
export function defineModule<TSettings extends z.ZodTypeAny = z.ZodTypeAny>(def: ModuleDefinition<TSettings>): ModuleDefinition<TSettings> {
  if (!/^[a-z][a-z0-9_]*$/.test(def.id)) throw new Error(`Invalid module id: ${def.id}`)
  return def
}
export function defineServerModule<TSettings extends z.ZodTypeAny = z.ZodTypeAny>(mod: ServerModule<TSettings>): ServerModule<TSettings> {
  return mod
}
...
export function toManifest(def: ModuleDefinition): ModuleManifest {
  return {
    ...
    permissions: (def.permissions ?? []).map((p) => ({ ...p, scope: p.scope ?? 'workspace', defaultRoles: p.defaultRoles ?? [], dangerous: p.dangerous ?? false })),
