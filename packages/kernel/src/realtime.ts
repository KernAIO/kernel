...
export const rtSubject = { channel: (ch: string) => `kern.rt.ch.${ch.replace(/:/g, '_')}`, user: (u: string) => `kern.rt.user.${u}` }

export function createRealtime(nats: NatsConnection | undefined, local?: (subject: string, msg: unknown) => void): Realtime {
  ...
  return {
    ...
    async toUsers(us, msg) { await Promise.all(us.map((u) => pub(rtSubject.user(u), msg))) },
