...
export interface PublishOptions { workspaceId?: string | null; actorId?: string | null; traceId?: string }
...
export interface EventBus {
  ...
  subscribe(pattern: string, handler: (event: EventEnvelope) => Promise<void> | void, opts?: { durable?: string }): Promise<Unsubscribe>
  close(): Promise<void>
}

export function makeEnvelope<E extends EventDef>(def: E, payload: EventPayload<E>, opts: PublishOptions = {}): EventEnvelope {
...
export class InMemoryEventBus implements EventBus {
  ...
  async publishRaw(event: EventEnvelope) {
    for (const s of this.subs) {
      ...
      try { await s.handler(event) } catch (err) { this.log?.error({ err, event: event.name }, 'event handler failed') }
    }
  }
...
export function uuidv7(): string {
  ...
  bytes[0] = Number((now >> 40n) & 0xffn); bytes[1] = Number((now >> 32n) & 0xffn); bytes[2] = Number((now >> 24n) & 0xffn)
  bytes[3] = Number((now >> 16n) & 0xffn); bytes[4] = Number((now >> 8n) & 0xffn); bytes[5] = Number(now & 0xffn)
